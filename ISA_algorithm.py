...
#update
    ...
    seed_indices=np.random.choice(m_conditions, size=seed_size,replace=False)
    ...
    for i in range(max_iter):
        ...
        # Otherwise, we calculate the z_scores normally
            ...
            gene_scores_z_scores=(gene_scores-gene_scores_mean)/gene_scores_standard_deviation
        ...
        if condition_scores_standard_deviation==0:
            ...
            condition_scores_z_scores=(condition_scores-condition_scores_mean)/condition_scores_standard_deviation

        s_c_new=(condition_scores_z_scores>=t_c).astype(int)
        if s_g_new.sum()==0 or s_c_new.sum()==0:
            raise ValueError("ISA: module collapsed — thresholds too high or seed too small")
